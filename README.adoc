...
// Build status badge
https://builds.gbif.org/job/globalnodes/lastBuild/console[image:https://builds.gbif.org/job/globalnodes/badge/icon[Build status]]
...
The English home page, https://regionalnodes.gbif-test.org/en/, is a component called `ROOT`, which is a special name where `ROOT` doesn't show in links or URLs.  Reserve pages in this section for the home page and very general pages about all the global nodes courses.
...
.Hiding a whole page from globalnodes.gbif.org — this **MUST** be at the start of the file
[source,asciidoc]
----
\ifeval::["{env}" == "prod"]
:page-unpublish:
...
for lang in en es; do
    docker run --rm -t --user $(id -u) --volume $PWD:/antora:Z antora/antora:3.0.1 globalnodes-$lang-playbook.yml
done
----
