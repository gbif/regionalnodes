...
= GBIF Global Nodes Training and Meeting

This is the the GBIF Global Nodes Training and Meeting site.

During development, it is visible at https://globalnodes.gbif-uat.org/

NOTE: This file is documenting the Global Nodes Training and Meeting site system, it's not shown on https://globalnodes.gbif-uat.org/.
...
Components:: Self-contained courses.
...
A version of a component is defined by its `antora.yml` descriptor file, which should be present at the location specified in `{url}/{start_path}` from the playbook's `content.sources` section.  For example, the https://globalnodes.gbif-uat.org/en/global-nodes/[_Global Nodes_] component has its descriptor file in this repository at link:./en/global-nodes/antora.yml[en/global-nodes/antora.yml]:
...
The English home page, https://globalnodes.gbif-uat.org/en/, is a component called `ROOT`, which is a special name where `ROOT` doesn't show in links or URLs.  Reserve pages in this section for the home page and very general pages about all the global nodes courses.
...
Pages or parts of pages can be excluded from globalnodes.gbif.org while they are under development.  This also excludes them from search results.
